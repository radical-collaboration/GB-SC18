...
def generate_pipeline(replica_ind):
    ...
    t4.arguments = ["%s/mineq_confs/eq0.conf" % rootdir]
    t4.cores = coresp
    t4.mpi = True

    t4.link_input_data = ['{stage2}/{input1}/replicas/rep{input2}/equilibration/holder > {input1}/replicas/rep{input2}/equilibration/holder'.format(stage2=stage_2_ref,input1 = rootdir, input2=replica_ind)]

    for f in my_list:
        t4.link_input_data.append("{stage2}/".format(stage2=stage_2_ref)+f+" > "+f)
    ...
    t5.arguments = ["%s/mineq_confs/eq0.conf" % rootdir]
    t5.cores = coresp
    t5.mpi = True

    t5.link_input_data = ['{stage2}/{input1}/replicas/rep{input2}/equilibration/holder > {input1}/replicas/rep{input2}/equilibration/holder'.format(stage2=stage_2_ref,input1 = rootdir, input2=replica_ind)]

    for f in my_list:
        t5.link_input_data.append("{stage2}/".format(stage2=stage_2_ref)+f+" > "+f)
    ...
    t6.arguments = ["%s/mineq_confs/eq0.conf" % rootdir]
    t6.cores = coresp
    t6.mpi = True

    t6.link_input_data = ['{stage2}/{input1}/replicas/rep{input2}/equilibration/holder > {input1}/replicas/rep{input2}/equilibration/holder'.format(stage2=stage_2_ref,input1 = rootdir, input2=replica_ind)]

    for f in my_list:
        t6.link_input_data.append("{stage2}/".format(stage2=stage_2_ref)+f+" > "+f)
    ...
    t7.arguments = ['-l', '-c', 'tar zxvf {input1} -C ./'.format(input1 = rootdir+".tgz")]
    t7.cores = 1
    t7.copy_input_data  = ["$SHARED/"+rootdir+".tgz > "+rootdir+".tgz"]

    stage_1_ref = "$Pipeline_{0}_Stage_{1}_Task_{2}/".format(p.uid, s1.uid, t7.uid)

    t7.download_output_data = ["{0}.tgz".format(rootdir)]
